use crate::{
    ...
    model::{Input, PodJob},
    orchestrator::{self, PodRun, PodRunAPI, RunInfo, RunState},
    store::ModelID,
};
use bollard::{
    container::{
...
pub struct LocalDockerOrchestrator {
    /// An absolute path on docker host where binds will be mounted from.
    pub data_directory: PathBuf,
    api: Docker,
    async_driver: Runtime,
}
/// API for to interact with the local docker orchestrator.
pub trait LocalDockerOrchestratorAPI: Sized {
    /// How to create a local docker orchestrator.
    ///
    /// # Errors
    ///
    /// Will return `Err` if there is an issue creating a local docker orchestrator.
    fn new(data_directory: impl AsRef<Path>) -> Result<Self>;
    /// Getter to promote reuse of the tokio runtime.
    fn get_async_driver(&self) -> &Runtime;
    /// Getter to promote reuse of a connection to docker daemon.
    fn get_api(&self) -> &Docker;
}

impl LocalDockerOrchestratorAPI for LocalDockerOrchestrator {
    fn new(data_directory: impl AsRef<Path>) -> Result<Self> {
        Ok(Self {
    ...
    fn list(&self) -> Result<Vec<PodRun<Self>>> {
        self.async_driver
            .block_on(list_containers(
                self,
                HashMap::from([("label".to_owned(), vec!["org.orcapod=true".to_owned()])]),
            ))?
            .map(|run_info| {
                PodRun::new(
    ...
    fn delete(&self, pod_run: &PodRun<Self>) -> Result<()> {
        self.async_driver.block_on(
            self.api.remove_container(
                &pod_run
                    .get_info()?
                    .ok_or(OrcaError::from(Kind::NonexistentPodRun {
                        model_id: pod_run.pod_job_model_id.clone(),
                    }))?
                    .name,
...
impl<'orch, T> PodRunAPI<T> for PodRun<'orch, T>
where
    T: LocalDockerOrchestratorAPI + Sync,
{
    fn get_info(&self) -> Result<Option<RunInfo>> {
        ...
        Ok(self
            ...
            .block_on(list_containers(
                self.orchestrator,
                HashMap::from([("label".to_owned(), labels)]),
            ))?
            .next())
    }
}
